...
class Solution:
    ...
    def numBusesToDestination(self, routes: list, S: int, T: int) -> int:
        if not routes:
            return -1

        bus_stops = defaultdict(list)
        for b, r in enumerate(routes):  # O(nm)
            for i, s in enumerate(r):
                if i == len(r) - 1:
                    bus_stops[s].append(BusRoute(s, b, r[0]))  # which stop, which bus, go to where
                else:
                    bus_stops[s].append(BusRoute(s, b, r[i+1]))

        # print(bus_stops)

        start = Travel(S, -1, -1, 0)
        queue = deque([start])
        minb = float('inf')
        bus_taken = set()
        stop_taken = set()
