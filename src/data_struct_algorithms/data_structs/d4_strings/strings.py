...
def partitionLabels(self, s: str) -> List[int]:
    max_idx = {letter: i for i, letter in enumerate(s)}  # O(n) time and space
    ret = []
    maxl = 0
    start = 0
...
def removeDuplicates(self, s: str, k: int) -> str:
    if not s: return ''
    n = len(s)
    def remove(s): # 2 pointers
        start, walker, counter = 0, 1, 0
        while walker < len(s):
            if k == walker - start:
                s = s[:start] + s[walker:]
                walker = start
            elif s[walker] != s[start]: start = walker
            walker += 1
        if k == walker - start: s = s[:start] + s[walker:] # end case
        return s
    while True:
        t = remove(s)
        if len(t) == len(s): return s  # no more removal
        s = t
...
def reorganizeString(self, s: str) -> str:
    ...
    if counts[keys[0]] > (n + 1) // 2: return "" # 3 chars can be separated by 2 different chars
    t = []
    for c in keys: t.extend(c * counts[c])
    t[::2], t[1::2] = t[:(n+1)//2], t[(n+1)//2:]  # left is same or one more than right
    return ''.join(t)
