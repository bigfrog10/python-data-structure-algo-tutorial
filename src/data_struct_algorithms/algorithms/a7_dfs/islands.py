...
def numDistinctIslands(self, grid: List[List[int]]) -> int:
    ...
    def explore(r, c, xo, yo, shape):
        if 0 <= r < n and 0 <= c < m and (r, c) not in explored and grid[r][c] != 0:
            explored.add((r, c))
            shape.add((r-xo, c-yo))
            explore(r+1, c, xo, yo, shape)
            explore(r-1, c, xo, yo, shape)
            explore(r, c+1, xo, yo, shape)
            explore(r, c-1, xo, yo, shape)
    shapes = set()
...
def islandPerimeter(self, grid: List[List[int]]) -> int:
    ...
    n, m = len(grid), len(grid[0])
    def dfs(i, j):
        if grid[i][j] == 0: return 0 # water
        if grid[i][j] == 2: return 0 # visited
        grid[i][j] = 2
        bound = 0
        for d in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            a, b = i + d[0], j + d[1]
            if a < 0 or a >= n or b < 0 or b >= m: bound += 1
            elif grid[a][b] == 0: bound += 1 # step on water from land
            elif grid[a][b] != 2: bound += dfs(a, b)
        return bound
    bound = 0
    for i in range(n):
        for j in range(m): bound += dfs(i, j)
    return bound
